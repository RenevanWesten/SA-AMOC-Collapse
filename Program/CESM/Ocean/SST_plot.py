#Program plots the SST difference
...
fh 	= netcdf.Dataset(directory+'Ocean/SST_fields.nc', 'r')

lon	    = fh.variables['lon'][:]
lat	    = fh.variables['lat'][:] 			
SST_1	= fh.variables['SST_0001-0050'][:] 
SST_2	= fh.variables['SST_2151-2200'][:] 
p_value = fh.variables['SST_diff'][:]
...
SST_plot			            = SST_2 - SST_1
SST_plot[SST_plot < -cut_off]	= (SST_plot[SST_plot < -cut_off] - -cut_off) / scale - cut_off
SST_plot[SST_plot > cut_off]	= (SST_plot[SST_plot > cut_off] - cut_off) / scale + cut_off
#-----------------------------------------------------------------------------------------

lon_1, lat_1, SST_1_plot, lon_2, lat_2, SST_2_plot, lon_3, lat_3, SST_3_plot, lon_4, lat_4, SST_4_plot	= LowCESMPlot(lon, lat, SST_plot)

fig, ax = plt.subplots(subplot_kw={'projection': ccrs.Robinson()})

CS      = ax.contourf(lon_1, lat_1, SST_1_plot, levels = np.arange(-6, 6.01, 0.25), extend = 'both', cmap = 'RdBu_r', transform=ccrs.PlateCarree())
CS      = ax.contourf(lon_2, lat_2, SST_2_plot, levels = np.arange(-6, 6.01, 0.25), extend = 'both', cmap = 'RdBu_r', transform=ccrs.PlateCarree())
CS      = ax.contourf(lon_3, lat_3, SST_3_plot, levels = np.arange(-6, 6.01, 0.25), extend = 'both', cmap = 'RdBu_r', transform=ccrs.PlateCarree())
CS      = ax.contourf(lon_4, lat_4, SST_4_plot, levels = np.arange(-6, 6.01, 0.25), extend = 'both', cmap = 'RdBu_r', transform=ccrs.PlateCarree())
...
for lat_i in range(0, len(lat), 10):
	for lon_i in range(0, len(lon[0]), 10):
		...
		if SST_plot[lat_i, lon_i] is ma.masked:
			continue
